  ...
  return (
    ...
    >
      <div className="container-custom flex items-center justify-between">
        {/* Logo */}
        <Link href="/">
          <span className="font-mono text-xl font-semibold text-primary terminal-cursor">
            {displayText}
          </span>
        </Link>
            ...
            <div className="container-custom py-4 flex flex-col space-y-4">
              {navItems.map((item) => (
                <Link 
                  key={item.name} 
                  href={item.path}
                  className={`text-lg py-2 ${pathname === item.path ? 'text-primary' : 'text-zinc-400'}`}
              ...
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer"
                className="button-outline inline-block text-center mt-2"
                onClick={() => setMobileMenuOpen(false)}
              >
                Resume
              </a>
